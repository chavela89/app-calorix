
export const general = {
  // General
  save: 'Save',
  cancel: 'Cancel',
  delete: 'Delete',
  edit: 'Edit',
  add: 'Add',
  remove: 'Remove',
  search: 'Search',
  filter: 'Filter',
  sort: 'Sort',
  view: 'View',
  more: 'More',
  less: 'Less',
  to: 'to',
  transfer_data: 'Transfer Data',
  delete_data: 'Delete Data',
  get_premium: 'Get Premium',
  learn_more: 'Learn More',
  start: 'Start',
  left: 'Left',
  duplicate: 'Duplicate',
  add_to_favorites: 'Add to Favorites',
  nutrition_analysis: 'Nutrition Analysis',
  per_100g: 'Per 100g',
  upgrade: 'Upgrade',
  consumed: 'Consumed',
  goal: 'Goal',
  search_recipes: 'Search recipes',
  all_recipes: 'All Recipes',
  vegetarian: 'Vegetarian',
  high_protein: 'High Protein',
  low_carb: 'Low Carb',
  quick_meal: 'Quick Meal',
  gluten_free: 'Gluten Free',
  dairy_free: 'Dairy Free',
  best: 'Best',
  active_days: 'Active Days',
  tracked_calories: 'Tracked Calories',
  goals_met: 'Goals Met',
  statistics: 'Statistics',
  weight_dynamics: 'Weight Dynamics',
  current_weight: 'Current Weight',
  target_weight: 'Target Weight',
  find_friends: 'Find Friends',
  members: 'Members',
  join_group: 'Join Group',
  view_all_challenges: 'View All Challenges',
  completed: 'Completed',
  active: 'Active',
  participants: 'Participants',
  whats_on_your_mind: "What's on your mind?",
  post: 'Post',
  add_photo: 'Add Photo',
  view_profile: 'View Profile',
  my_account: 'My Account',
  error: 'Error',
  success: 'Success',
  warning: 'Warning',
  info: 'Information',
  recipe_name_required: 'Recipe name is required',
  recipe_loaded: 'Recipe loaded',
  previous_day: 'Previous day',
  next_day: 'Next day',
  enter_valid_amount: 'Please enter a valid amount',
  ingredient: 'Ingredient',
  feature_coming_soon: 'This feature is coming soon',
  search_food_or_scan: 'Search food or scan barcode',
  month: 'Month',
  day: 'Day',
  week: 'Week',
  year: 'Year',
  loading: 'Loading...',
  no_data: 'No data',
  toggle_menu: 'Toggle menu',
  premium: 'Premium',
  calorie_tracking: 'Calorie tracking',
  calorie_tracking_description: 'Track your calorie intake',
  meal_planning: 'Meal planning',
  meal_planning_description: 'Plan your meals',
  progress_tracking: 'Progress tracking',
  progress_tracking_description: 'Track your progress',
  basic_recipes: 'Basic recipes',
  basic_recipes_description: 'Access to basic recipes',
  water_tracking: 'Water tracking',
  water_tracking_description: 'Track your water intake',
  saved_recipes: 'Saved recipes',
  load_recipe: 'Load recipe',
  cost: 'Cost',
  servings: 'Servings',
  min: 'min',
};
